  ...
  return (
    <div className="date-selector-container">
      ...
      <div className="selected-dates">
        <p>
          시작 날짜: {startDate ? `${startDate.getFullYear()}-${String(startDate.getMonth() + 1).padStart(2, "0")}-${String(startDate.getDate()).padStart(2, "0")}` : "선택되지 않음"}
        </p>
        <p>
          종료 날짜: {endDate ? `${endDate.getFullYear()}-${String(endDate.getMonth() + 1).padStart(2, "0")}-${String(endDate.getDate()).padStart(2, "0")}` : "선택되지 않음"}
        </p>
      </div>
        <button
