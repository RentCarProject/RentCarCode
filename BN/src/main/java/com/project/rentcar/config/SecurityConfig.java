...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
        ...
        return httpSecurity.csrf((config) -> {config.disable();})
                ...
                .authorizeHttpRequests((auth) -> {
                    auth.requestMatchers("/api/v1/auth/**","/","/getToken","/api/v1/reviews/**","/open/rentcar","/api/cars/**","/api/portone/**","/api/v1/rentals/**","/api/cars/available").permitAll();
    ...
    CorsConfigurationSource corsConfigurationSource(){
        ...
        config.setAllowedHeaders(Arrays.asList("Content-Type", "Authorization", "X-XSRF-TOKEN","REFRESH_TOKEN"));
